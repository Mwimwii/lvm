#!/usr/bin/env python3
"""
Lecture Video Generator — Nano-vLLM VoxCPM backend (with reference voice cloning)
Platform-agnostic Gradio UI using the high-performance nanovllm-voxcpm engine.

Usage:
    python app_nanovllm.py              # serve on 0.0.0.0:9999
    python app_nanovllm.py --share      # serve with Gradio share tunnel

Prerequisites:
    Linux + NVIDIA GPU (CUDA)
    pip install -r requirements-nanovllm.txt

Note: nanovllm-voxcpm requires model weights in *.safetensors format.
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
SAMPLE_RATE = 48_000         # VoxCPM2 output sample rate (nanovllm backend)
...
def _find_default_ref_wav() -> str | None:
    ...
    candidates.extend([
        os.path.join(_BASE, "vo", "speaker.wav"),
        os.path.join(os.path.dirname(_BASE), "vo", "speaker.wav"),
        "/content/vo/speaker.wav",
    ])
    for p in candidates:
...
cached_ref_latents: bytes | None = None  # encoded prompt_latents from AudioVAE
...
def _encode_ref_audio(wav_path: str) -> bytes:
    """Encode a reference WAV file into prompt_latents bytes via the server VAE."""
    assert server is not None
    # Read raw WAV bytes
    with open(wav_path, "rb") as f:
        wav_bytes = f.read()
    # nanovllm encode_latents expects raw audio bytes + format string
    latents = server.encode_latents(wav_bytes, "wav")
    return latents


def _generate_segment(text: str, cfg: float, _timesteps: int) -> np.ndarray:
    """Generate audio for one text segment using the nanovllm sync server."""
    chunks = []
    kwargs = {"target_text": text, "cfg_value": cfg}
    if cached_ref_latents is not None:
        kwargs["prompt_latents"] = cached_ref_latents
        kwargs["prompt_text"] = cached_ref_text or ""
...
def save_reference(audio_file: str | None, text: str) -> str:
    global cached_ref_wav, cached_ref_text, cached_ref_latents
    ...
    # Encode reference audio into latents via the server VAE
    try:
        cached_ref_latents = _encode_ref_audio(dest)
        size_mb = os.path.getsize(dest) / 1024 / 1024
        return (
            f"✅ Reference saved & encoded: `{dest}` ({size_mb:.1f} MB)\n\n"
            f"**Transcript:** {cached_ref_text[:120]}{'…' if len(cached_ref_text) > 120 else ''}\n\n"
            f"Latents shape: {len(cached_ref_latents)} bytes"
        )
    except Exception as e:
        cached_ref_latents = None
        return f"⚠️ Saved audio but encoding failed: {e}"


def get_ref_status() -> str:
    parts = []
    if cached_ref_latents is not None:
        parts.append(f"🔐 Latents cached: {len(cached_ref_latents)} bytes")
    if cached_ref_wav and os.path.exists(cached_ref_wav):
        size_mb = os.path.getsize(cached_ref_wav) / 1024 / 1024
        parts.append(f"📎 Audio file: `{cached_ref_wav}` ({size_mb:.1f} MB)")
        parts.append(f"**Transcript:** {cached_ref_text[:120]}{'…' if len(cached_ref_text) > 120 else ''}")
    elif DEFAULT_REF_WAV and os.path.exists(DEFAULT_REF_WAV):
        parts.append(f"📎 Default reference ready: `{DEFAULT_REF_WAV}`")
        parts.append("Upload a custom voice above to override.")
    else:
        parts.append("⚠️ No reference voice found.")
        parts.append("Upload a .wav + transcript, clone the `vo` repo, or set `REF_VOICE_PATH`.")
...
def _run_pipeline(
    ...
    yield (
        ...
        f"Segments: **{total_segments}**\n\n🔊 Generating audio (nanovllm + voice clone)...",
    )
...
def build_ui() -> gr.Blocks:
    with gr.Blocks(title="Lecture Video Generator (nanovllm)") as demo:
        gr.Markdown("# 🎓 Lecture Video Generator")
        gr.Markdown(
            "High-performance backend powered by **Nano-vLLM VoxCPM** with voice cloning.\n\n"
        ...
        preview_btn.click(
            ...
            inputs=[
                script_input, pdf_input,
                cfg_slider, timesteps_slider, video_height,
                clear_checkbox, preview_start, preview_end,
            ],
            outputs=[output_video, output_audio, status_md],
            show_progress="minimal",
        )
...
# ═══════════════════════════════════════════════════════════════════════════════
def main() -> None:
    parser = argparse.ArgumentParser(description="Lecture Video Generator UI (nanovllm)")
    parser.add_argument("--share", action="store_true", help="Create a public Gradio share link")
    parser.add_argument(
        "--model", default="openbmb/VoxCPM2",
        help="HuggingFace repo id or local path to VoxCPM2 weights"
    )
    parser.add_argument(
        "--gpu-mem", type=float, default=0.95,
    ...
    if not torch.cuda.is_available():
        ...
        print("")
        print("nanovllm-voxcpm requires a GPU with CUDA support.")
        print("Current PyTorch CUDA version:", torch.version.cuda)
        print("=" * 70)
        sys.exit(1)

    print(f"CUDA: {torch.cuda.get_device_name(0)}")
    print(f"Loading nanovllm VoxCPM server (model={args.model}, gpu_mem={args.gpu_mem})...")

    global server, model_info
    ...
    print(f"Model info: sample_rate={model_info['sample_rate']}, "
          f"encoder_sample_rate={model_info['encoder_sample_rate']}, "
          f"feat_dim={model_info['feat_dim']}")

    # Pre-encode default reference voice if available
    global cached_ref_latents, cached_ref_wav, cached_ref_text
    if DEFAULT_REF_WAV and os.path.exists(DEFAULT_REF_WAV):
        print(f"Pre-encoding default reference voice: {DEFAULT_REF_WAV}")
        cached_ref_wav = DEFAULT_REF_WAV
        cached_ref_text = DEFAULT_REF_TEXT
        try:
            cached_ref_latents = _encode_ref_audio(DEFAULT_REF_WAV)
            print(f"Reference latents cached: {len(cached_ref_latents)} bytes")
        except Exception as e:
            print(f"Warning: failed to encode default reference: {e}")
            cached_ref_latents = None
